...
const MapComponent = ({ startPosition, endPosition }) => {
  ...
  //     navigate("/"); //Redirect back to the input form page

  //   }
  // },[startPosition,endPosition,navigate]);

  useEffect(() => {
    const fetchLocationName = async (lat, lng, setLocationName) => { // <--- Added function for reverse geocoding
      try {
    ...
    if (startPosition && endPosition) {
      fetchLocationName(startPosition.lat, startPosition.lng, setStartLocationName); // Fetch start location name
  ...
  return (
   ...
   <div className="flex flex-col place-content-center items-start border w-auto p-5 bg-gray-00 border-blue-600 shadow-2xl m-5 rounded-lg outline-none">
      <label><span className="font-bold text-green-700">Your Start Position:</span> {startLocationName}</label>
      <label><span className="font-bold text-red-700">Your End Position:</span> {endLocationName}</label>
    </div>
     <div className=" m-5 w-[80%] flex items-center place-content-center flex-col border-black shadow-2xl ">
      <MapContainer className="flex items-center place-content-center" center={vehiclePosition} zoom={8} style={{ height: "100vh", width: "100%" }}>
        <TileLayer
          url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
          attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
        />
        <Marker position={vehiclePosition} icon={vehicleIcon}></Marker> {/* Moving vehicle */}
        <Marker position={startPosition} icon={startFlagIcon}></Marker> {/* Green flag for start */}
        <Marker position={endPosition} icon={endFlagIcon}></Marker> {/* Red flag for end */}
        <Polyline positions={path} color="skyblue" />
      </MapContainer>
      <div>
      <a target="_blank" href="https://icons8.com/icon/65545/green-flag">Green and red Flag</a> icon by <a target="_blank" href="https://icons8.com">Icons8</a>
